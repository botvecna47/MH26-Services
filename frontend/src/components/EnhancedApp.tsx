...
function ErrorFallback({ error, resetErrorBoundary }: { error: AppError; resetErrorBoundary: () => void }) {
  useEffect(() => {
    // Error logged to error tracking service (e.g., Sentry)
  ...
  useEffect(() => {
    ...
    if ('performance' in window) {
      ...
      if (navigation) {
        ...
        if (loadTime > 3000) {
          // Slow page load detected - log to monitoring service
        }
      }
    }
...
export default function EnhancedApp() {
  ...
  useEffect(() => {
    const initializeApp = async () => {
      try {
        ...
        // App initialization error - log to error tracking service
  ...
  const handleLogout = async () => {
    try {
      ...
      // Logout error - log to error tracking service
  ...
  return (
    <ErrorBoundary
      ...
      onError={(error, errorInfo) => {
        // Error Boundary caught an error - log to error tracking service
      }}
      onReset={() => {
